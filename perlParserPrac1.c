...
struct Token getNextToken(FILE *fin) {
    ...
    while ((c = fgetc(fin)) != EOF) {
        ...
        if (isSpecialSymbol(c)) { // Handle special symbols
            token.lexeme[0] = c;
            token.lexeme[1] = '\0';
        ...
        if (isdigit(c)) { // Handle numbers
            int i = 0;
            token.lexeme[i++] = c;
            while (isdigit(c = fgetc(fin))) token.lexeme[i++] = c;
            token.lexeme[i] = '\0';
        ...
        if (isalpha(c) || c == '_') { // Handle keywords and identifiers
            int i = 0;
            token.lexeme[i++] = c;
            while (isalnum(c = fgetc(fin)) || c == '_') token.lexeme[i++] = c;
            token.lexeme[i] = '\0';
            fseek(fin, -1, SEEK_CUR);
